...
drawbarwin(Bar *bar)
{
	...
	bar->scheme = getschemefor(bar->mon, groupactive, bar->mon == selmon);
...
{
	if (!selmon->layout || (selmon->sel && ISFLOATING(selmon->sel)))
		movemouse(arg);
	else
		placemouse(arg);
}
...
setup(void)
{
	...
	XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8,
		PropModeReplace, (unsigned char *) "dawn", 4);
...
showhide(Client *c)
{
	...
	if (ISVISIBLE(c) && !HIDDEN(c)) {
		...
		if ((ISFLOATING(c) || !c->mon->layout->arrange) && c->sfx != -9999 && (!ISFULLSCREEN(c) || ISFAKEFULLSCREEN(c))) {
			restorefloats(c);
			showhide(c->snext);
			return;
		}
